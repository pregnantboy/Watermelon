...
@property (nonatomic, strong) NSArray *valueViews;
@property (nonatomic, strong) NSArray *lowervalueViews;
...
- (void)reloadData
{
    ...
    dispatch_block_t createBars = ^{
        ...
        CGFloat xOffset = 0;
        NSUInteger index = 0;
        NSMutableArray *mutablevalueViews = [NSMutableArray array];
        NSMutableArray *mutablelowervalueViews = [NSMutableArray array];
        ...
        for (NSNumber *key in [[self.chartDataDictionary allKeys] sortedArrayUsingSelector:@selector(compare:)])
        {
           ...
           int rawheight = [[self.chartDataDictionary objectForKey:key]intValue];
            int gallonssaved = [self maximumValue] - height;
            
            barView.frame = CGRectMake(xOffset, self.bounds.size.height - height - self.footerView.frame.size.height, [self barWidth], height);
            barView.layer.cornerRadius = [self barWidth]/2;
           ...
           [mutableCachedBarViewHeights addObject:[NSNumber numberWithFloat:height]];
			...
			else
			{
                ...
                [mutablevalueViews addObject:valuelabel];
                [self addSubview: valuelabel];
                if ([self.valueViews objectAtIndex:index]!=NULL){
                    [[self.valueViews objectAtIndex:index] removeFromSuperview];
                }
                
                UILabel *lowervaluelabel =[[UILabel alloc]initWithFrame:CGRectMake(xOffset+([self barWidth]/2)-20, self.bounds.size.height - height - self.footerView.frame.size.height+5 , 40,20)];
                
                
                lowervaluelabel.text = [NSString stringWithFormat: @"%d", rawheight];
                
                lowervaluelabel.font = [valuelabel.font fontWithSize:12];
                lowervaluelabel.textAlignment = UITextAlignmentCenter;
                lowervaluelabel.textColor = [UIColor blackColor];
                [mutablelowervalueViews addObject:lowervaluelabel];
                [self addSubview: lowervaluelabel];

                
                if ([self.lowervalueViews objectAtIndex:index]!=NULL){
                    [[self.lowervalueViews objectAtIndex:index] removeFromSuperview];
                }
                
                [ self insertSubview:barView belowSubview:valuelabel];
			}
            xOffset += ([self barWidth] + self.barPadding);
            index++;
        }
        self.lowervalueViews = [NSArray arrayWithArray:mutablelowervalueViews];
        self.valueViews = [NSArray arrayWithArray:mutablevalueViews];
    ...
    // Refresh state
    [self setState:self.state animated:YES force:YES callback:nil];
}
